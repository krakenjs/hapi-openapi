        ...
        if (openapi && requestBody && requestBody.content) {
            ...
            for (const mediaType of consumes) {
                ...
                if (requestBody.content[mediaType].schema) {
                    ...
                    validate.payload = (body, ctx) => {
                        let contentType = ctx.context.headers["content-type"];
                        if(consumes.includes(contentType) && contentType === "text/x-yaml") {
                            body =  yaml.load(a[0]);
                        }
                        return validator.validate(body, context);
                    }
                    break;
                }
            }
