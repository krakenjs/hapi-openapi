...
const register = async function (server, options, next) {
    ...
    const { api, cors, vhost, handlers, extensions } = validation.value;
    ...
    if (spec['x-auth-schemes']) {
        for (const [name, path] of Entries(spec['x-auth-schemes'])) {
    ...
    if (spec.securityDefinitions) {
        for (const [name, security] of Entries(spec.securityDefinitions)) {
            if (security['x-auth-strategy']) {
                const strategy = require(Path.resolve(Path.join(basedir, security['x-auth-strategy'])));
