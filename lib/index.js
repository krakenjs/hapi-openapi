...
const ApiDtoMapper = require('./apiDtoMapper');
...
const registerCustomAuth = async (server, basedir, apiDto) => {

    apiDto.customAuthSchemes.forEach(async ([name, path]) => {
    ...
    apiDto.customAuthStrategies.forEach(async ([name, security]) => {
...
const registerDocsPath = (server, docs, docspath, apiDocument, basePath, cors, vhost) => {
...
const register = async function (server, options, next) {
    ...
    const apiDto = ApiDtoMapper.toDto(spec);

    await registerCustomAuth(server, basedir, apiDto);

    registerDocsPath(server, docs, docspath, apiDocument, spec.basePath, cors, vhost);
