...
export const create = async function (
  server,
  { api, basedir, cors, vhost, handlers, extensions, outputvalidation },
) {
  const routes = [];
  ...
  if (!handlers) {
    for (const [path, operations] of Object.entries(api.paths)) {
      if (operations["x-hapi-handler"]) {
        ...
        const xhandler = await import(
          Path.resolve(Path.join(basedir, operations["x-hapi-handler"]))
        );
  ...
  for (const [path, operations] of Object.entries(api.paths)) {
    ...
    for (const [method, operation] of Object.entries(operations)) {
      ...
      if ((operation.parameters || operation.requestBody) && !skipValidation) {
        const allowUnknownProperties = xoptions.validate &&
      ...
      if (operation.security && operation.security.length) {
        for (const secdef of operation.security) {
          ...
          for (const securityDefinitionName of securitySchemes) {
            ...
            if (api.swagger) {
              securityDefinition =
                api.securityDefinitions[securityDefinitionName];
            }

            if (api.openapi) {
              securityDefinition =
                api.components.securitySchemes[securityDefinitionName];
            }
