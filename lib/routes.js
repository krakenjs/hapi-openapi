...
const create = async function (server, { api, basedir, cors, vhost, handlers, extensions }) {
    const routes = [];
    ...
    //Support x-handler when no handlers set. Build handlers from x-handlers.
    if (!handlers) {
        for (const [path, operations] of Entries(api.paths)) {
            if (operations['x-handler']) {
                const pathnames = path.split('/').slice(1).join('.');
                if (!handlers) {
                    handlers = {};
                }
                const xhandler = require(Path.resolve(Path.join(basedir, operations['x-handler'])));
